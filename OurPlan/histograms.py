...
sampling_grayST = "YOLO_MJO_grayST.xlsx"
sampling_grayST_k = "YOLO_MJO_grayST_k100.xlsx"


def summary(path_to_metrics, name, path_to_result_excels):
    ...
    for i in range(1, 31):
        ...
        nb_filename = os.path.join(path_to_metrics, f"metrics_{i}.xlsx")
    ...
    summary_filename = os.path.join(path_to_result_excels, name)
...
def predict_on_4_predictions(video_labels, confidence_levels):
    y_vid_test = []
    y_vid_pred = []
    ...
    for video, predictions in video_labels.items():
        ...
        for i, pred in enumerate(labels_pred):
            ...
            index_class = class_labels.index(labels_test[i])
            confidence_levels[index_class].append(pred[index_class])
...
def histogram(confidence_levels, i, save_directory):
    # Create a histogram
    plt.hist(confidence_levels, bins=20, edgecolor='black')

    # Set labels for x and y axes
    plt.xlabel('Confidence level (%)')
    plt.ylabel('Frames')

    save_path = os.path.join(save_directory, f'{class_labels[i]}.jpg')
...
confidence_levels = np.zeros(4, dtype=object)
confidence_levels[0] = []
confidence_levels[1] = []
confidence_levels[2] = []
confidence_levels[3] = []
...
for i in range(i_start, i_end + 1):
    ...
    y_vid_test, y_vid_pred = predict_on_4_predictions(video_labels, confidence_levels)
...
histogram(confidence_levels[0], 0, save_directory_path)
histogram(confidence_levels[1], 1, save_directory_path)
histogram(confidence_levels[2], 2, save_directory_path)
histogram(confidence_levels[3], 3, save_directory_path)

summary(metrics_sampling_grayST_path, sampling_grayST, result_excels_path)
...
#         #
#         #
#         # majority_vote_pred_k, counter_k = most_common_element_and_count(labels_pred_k)
#         #
#         # # Create a dictionary to store the counts
#         # counts_dict = {
#         #     majority_vote_pred_k: counter_k
#         # }
#         one_preds = []
#         for l, pred in enumerate(labels_pred_k):
#             one_preds.append(class_labels[max(range(len(pred)), key=pred.__getitem__)])
#             index_class = class_labels.index(labels_test[l])
#             confidence_levels[index_class].append(pred[index_class])
#
...
#
#     histogram(confidence_levels[0], 0, save_directory_horse_path)
#     histogram(confidence_levels[1], 1, save_directory_horse_path)
#     histogram(confidence_levels[2], 2, save_directory_horse_path)
#     histogram(confidence_levels[3], 3, save_directory_horse_path)
#
#     print(f"LOOCV iteration {i}: Files saved successfully.")
#
#
# summary(metrics_sampling_grayST_k_path, sampling_grayST_k, result_excels_path)