...
def train_the_model(metadata_dir, new_metadata_dir, metrics_dir, new_metrics_dir, class_labels, k, k_baseline, i_start, i_end, retrain=False):
    ...
    for i in range(i_start, i_end + 1):
        ...
        if not retrain:
            ...
            top_k_1 = [(-1, 0) for _ in range(k_baseline)]
        ...
        for index, row in metadata.iterrows():
            ...
            if not retrain:
                ...
                if label_pred[1] > top_k_1[k_baseline - 1][0] and label_test == class_labels[1]:
                    top_k_1[k_baseline - 1] = (label_pred[1], index)
        ...
        for video, predictions in video_labels.items():
            ...
            sum_pred = [sum(items) for items in zip(*labels_pred)]
            pred = sum_pred.index(max(sum_pred))

            average_confidence_level_pred = class_labels[pred]
            average_confidence_level_test = max(set(labels_test), key=labels_test.count)

            y_vid_test.extend([average_confidence_level_test])
            y_vid_pred.extend([average_confidence_level_pred])
