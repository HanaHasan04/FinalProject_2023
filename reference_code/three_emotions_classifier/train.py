...
def concatenate_metadata(all_metadata_path, i_start, i_end):
    metadata_list = []
...
def train_the_model(metadata_dir, new_metadata_dir, metrics_dir, new_metrics_dir, class_labels, k, k_baseline, i_start, i_end, retrain=False):
    ...
    if retrain:
        all_metadata = concatenate_metadata(new_metadata_dir, i_start, i_end)
    else:
        all_metadata = concatenate_metadata(metadata_dir, i_start, i_end)
    ...
    for i in range(i_start, i_end + 1):
        ...
        video_labels = {}
        if not retrain:
            top_k_0 = [(-1, 0) for _ in range(k_baseline)]
        ...
        for index, row in metadata.iterrows():
            ...
            if not retrain:
                ...
                if label_pred[0] > top_k_0[k_baseline - 1][0] and label_test == class_labels[0]:
                    top_k_0[k_baseline - 1] = (label_pred[0], index)
